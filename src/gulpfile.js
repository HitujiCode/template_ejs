...
const cssSass = () => {
  ...
  return (
    src(srcPath.css)
      ...
      .pipe(postcss([cssdeclsort({ order: "alphabetical" }), postcssPresetEnv(browsers)]))
      // メディアクエリを統合
      .pipe(mmq())
      // ソースマップを書き出し
...
const ejsCompile = (done) => {
  ...
  src([srcEjsDir + "/**/*.ejs", "!" + srcEjsDir + "/**/_*.ejs", "!" + srcEjsDir + "/tutorial/**/*.ejs"])
    // エラーハンドリングを設定
    .pipe(
      plumber({
